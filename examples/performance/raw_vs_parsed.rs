...
use kiteticker_async_manager::{as_tick_raw, Tick, TickRaw};
...
fn main() {
  ...
  assert_eq!(
    bytes.len(),
    184,
    "benchmark expects a 184-byte full payload"
  );
  ...
  for _ in 0..iterations {
    ...
    if let Some(lp) = t.last_price {
      checksum_p = checksum_p.wrapping_add(black_box(lp.to_bits()));
    }
  }
  ...
  for _ in 0..iterations {
    ...
    checksum_r = checksum_r
      .wrapping_add(black_box(v.header.instrument_token.get() as u64));
    checksum_r =
      checksum_r.wrapping_add(black_box(v.header.last_price.get() as u64));
  }
  ...
  println!(
    "  parsed:   total {:?}, per op: {:.3} µs ({:.0} ns), throughput: {:.0}/s",
    elapsed_p,
    per_parsed * 1e6,
    per_parsed * 1e9,
    (iterations as f64) / elapsed_p.as_secs_f64()
  );
  println!(
    "  raw view: total {:?}, per op: {:.3} µs ({:.0} ns), throughput: {:.0}/s",
    elapsed_r,
    per_raw * 1e6,
    per_raw * 1e9,
    (iterations as f64) / elapsed_r.as_secs_f64()
  );
