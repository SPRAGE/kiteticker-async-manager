...
async fn main() -> Result<(), String> {
  ...
  println!(
    "💡 Note: This demo shows enhanced tick printing with detailed market data"
  );
        ...
        match receiver.recv().await {
          Ok(message) => {
            if let TickerMessage::Ticks(ticks) = message {
              ...
              println!(
                "🎯 PERMANENT LISTENER {:?}: {} ticks (total: {})",
                channel_id,
                ticks.len(),
                total_ticks
              );
              ...
              for (i, tick) in ticks.iter().enumerate() {
                ...
                debug!(
                  "DEBUG Full Tick Data for symbol {}:",
                  tick.instrument_token
                );
                ...
                debug!(
                  "  - Exchange Timestamp: {:?}",
                  tick.content.exchange_timestamp
                );
                ...
                if tick.content.mode == Mode::Full {
                  if let Some(depth) = &tick.content.depth {
                    println!(
                      "      📈 Market Depth: {} buy orders, {} sell orders",
                      depth.buy.len(),
                      depth.sell.len()
                    );
                    ...
                    debug!(
                      "Market depth for symbol {}: {} buy, {} sell",
                      tick.instrument_token,
                      depth.buy.len(),
                      depth.sell.len()
                    );
                    ...
                    if !depth.buy.is_empty() {
                      debug!(
                        "Top Buy Orders for symbol {}:",
                        tick.instrument_token
                      );
                      for (idx, buy_order) in
                        depth.buy.iter().take(3).enumerate()
                      {
                        debug!(
                          "  {}. Price: {}, Qty: {}, Orders: {}",
                          idx + 1,
                    ...
                    if !depth.sell.is_empty() {
                      debug!(
                        "Top Sell Orders for symbol {}:",
                        tick.instrument_token
                      );
                      for (idx, sell_order) in
                        depth.sell.iter().take(3).enumerate()
                      {
                        debug!(
                          "  {}. Price: {}, Qty: {}, Orders: {}",
                          idx + 1,
                    ...
                    debug!(
                      "Market Depth: Not available for symbol {}",
                      tick.instrument_token
                    );
                  }
                }
...
async fn monitor_ticks_briefly(
  manager: &mut KiteTickerManager,
  duration_secs: u64,
  context: &str,
) {
  println!(
    "\n📺 {} - Monitoring ticks for {} seconds...",
        ...
        match timeout(Duration::from_secs(2), receiver.recv()).await {
          Ok(Ok(message)) => {
            ...
            if let TickerMessage::Ticks(ticks) = message {
              ...
              debug!(
                "Monitoring tick batch #{} with {} ticks on {:?}",
                count,
                ticks.len(),
                channel_id
              );
              ...
              for (idx, tick) in ticks.iter().enumerate() {
                ...
                debug!(
                  "  - Buy/Sell Qty: {:?}/{:?}",
                  tick.content.total_buy_qty, tick.content.total_sell_qty
                );

                if tick.content.mode == Mode::Full
                  || tick.content.mode == Mode::Quote
                {
                  if tick.content.ohlc.is_some() {
                    debug!(
  ...
  for task in tasks {
    if let Ok((channel_id, count)) = task.await {
      println!(
        "📊 {:?}: {} total messages in {} seconds",
        channel_id, count, duration_secs
      );
    }
  }
}
...
async fn demonstrate_offline_dynamic_architecture() {
  ...
  println!(
    "   manager.subscribe_symbols(&[408065, 884737], Some(Mode::Full)).await?;"
  );
  println!("   ");
